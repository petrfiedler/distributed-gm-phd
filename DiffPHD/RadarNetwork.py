...
class RadarNetwork:
    ...
    def target_in_fov(self, target: np.ndarray) -> bool:
        """
        Check if a target is in the field of view of any radar.
        """
        for radar in self.radars:
            if np.linalg.norm(target[:2] - radar.center) <= radar.radius:
                return True
    ...
    def evolve_targets(self):
        ...
        for target, trajectories in zip(self.targets, self.target_trajectories):
            ...
            if np.random.rand() < self.survival_probability and self.target_in_fov(
                target
            ):
    ...
    def get_radar(self, id: str) -> Radar:
        """
        Get a radar by its id.
        """
        return next(radar for radar in self.radars if radar.id == id)
    ...
    def plot_radars(
        ...
        if graph is not None:
            ...
            for radar in self.radars:
                for neighbor in graph[radar.id]:
                    if (radar.id, neighbor) in distinct_pairs:
                        continue
                    distinct_pairs.append((radar.id, neighbor))
                    radar_a = self.get_radar(radar.id)
                    radar_b = self.get_radar(neighbor)
                    plt.plot(
        ...
        for radar in self.radars:
            ...
            plt.text(radar.center[0] + 0.25, radar.center[1] + 0.25, radar.id)
        plt.grid(False)
